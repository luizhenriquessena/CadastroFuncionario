...
procedure TfrmCadFuncionario.btnLimparClick(Sender: TObject);
begin
  pLimparCamposCadFuncionario;
end;
...
procedure TfrmCadFuncionario.medNascimentoExit(Sender: TObject);
var d_Data : TDateTime;
begin
  if medNascimento.Text <> '  /  /    ' then
   begin
    try
      d_Data := StrToDateTime(medNascimento.Text);
      medNascimento.Text := FormatDateTime('dd/mm/yyyy',d_Data);
    except
      ShowMessage('Data inválida');
      medNascimento.Text := '  /  /    ';
      medNascimento.SetFocus;
    end;
  end;
end;
...
begin
  ...
  d_Data := StrToDate(medNascimento.Text);
  frmMain.FFuncionario.Nascimento     :=  FormatDateTime('yyyy-mm-dd',d_Data);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      SQL.Add(',nascimento          = ' +#39+frmMain.FFuncionario.Nascimento+#39);
...
procedure TfrmCadFuncionario.pIncluirDados;
begin
  ...
  try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('insert into funcionario (nomeFuncionario, sexo, idCidade, idBairro, idDepartamento, idCargo, salario, nascimento)');
      SQL.Add('values ( ');
      SQL.Add(#39+ frmMain.FFuncionario.Nome +#39);
      SQL.Add(','+#39+ frmMain.FFuncionario.Sexo +#39);
      SQL.Add(','+ IntToStr(frmMain.FFuncionario.IdCidade));
      SQL.Add(','+ IntToStr(frmMain.FFuncionario.IdBairro));
      SQL.Add(','+ IntToStr(frmMain.FFuncionario.IdDepartamento));
      SQL.Add(','+ IntToStr(frmMain.FFuncionario.IdCargo));
      SQL.Add(','+ CurrToStr(frmMain.FFuncionario.Salario));
      SQL.Add(','+#39+ frmMain.FFuncionario.Nascimento+#39+')');
      ExecSQL;
    end;
  except
    on E: Exception do
...
begin
  ...
  if medNascimento.Text = '  /  /    ' then
  begin
    ShowMessage('Favor informar a data de nascimento do funcionário');
    Abort;
  end;
end;
